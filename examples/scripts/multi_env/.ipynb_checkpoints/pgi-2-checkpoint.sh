...
do
    ...
    do
        for ADAPT_WEIGHT in 1e-3 1e-2 1e-1
        do
            for ASCEND_RATE in 0.3 0.6
            do
                OUTPUT_FOLDER=examples/histories/multi_env/${METHOD}/basic_bert_lr_5_epoch_3_lambda_${ADAPT_WEIGHT}_${ASCEND_RATE}-r-SEED-${SEED}/
                ...
                if [ ! -f "$FILE" ]; then
                    CUDA_VISIBLE_DEVICES=$CUDA_DEVICE READER_DEBUG=0 python -W ignore::UserWarning __main__.py multi_environment_train \
                        ...
                        --loss-args '{"weight_adapt":'${ADAPT_WEIGHT}',"reverse":true}' \
