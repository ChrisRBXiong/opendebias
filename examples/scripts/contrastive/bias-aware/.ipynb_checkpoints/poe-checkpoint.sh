...
# $6 DEBIAS_ARGS
{
    ...
    # Train Model
    if [ ! -f "$FILE" ]; then
        CUDA_VISIBLE_DEVICES=$CUDA_DEVICE READER_DEBUG=1 python -W ignore::UserWarning __main__.py assemble_debiased_train \
            ...
            --sampler bias-aware-same-class-negative  --sampler-args ${SAMPLER_ARGS} \
...
OUTPUT_BASE_FOLDER=examples/histories/contrastive/mnli/
        ...
        do
            ...
            do
                ...
                do
                    ...
                    OUTPUT_FOLDER=${OUTPUT_BASE_FOLDER}/bias-aware/basic_bert-lr-5-KP-${K_POS}-KN-${K_NEG}-${REAL_BATCH_SIZE}-tau-${TAU}-lamda-${LAMDA}-SEED-${SEED}
