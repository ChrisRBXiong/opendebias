...
from allennlp.training import Trainer, BatchCallback, EpochCallback
from allennlp.models.archival import load_archive

logger = logging.getLogger(__name__)

@Trainer.register("opendebias_gradient_descent", constructor="from_partial_objects")
class OpenDebiasGradientDescentTrainer(Trainer):
    """
    1. show learning rate
    2. cooperate with auto linear decay with warmup scheduler
    """
    ...
    def __init__(
        ...
        if hasattr(learning_rate_scheduler, "need_reconstruct"):
            self._learning_rate_scheduler.construct(
                num_epochs=num_epochs,
                num_steps_per_epoch=math.ceil(len(self.data_loader) / num_gradient_accumulation_steps))
        ...
        self._tensorboard = tensorboard_writer or TensorboardWriter(serialization_dir, should_log_learning_rate=False)
...
#                 print([d['id'] for d in batch['metadata']])
            ...
            if self._master:
                ...
                description = description[:-2] + "lr: {:.2e}".format(rate) + " ||"
...
#                 print([d['id'] for d in batch['metadata']])
    ...
    def from_partial_objects(
        ...
        batch_callbacks: List[BatchCallback] = None,
        epoch_callbacks: List[EpochCallback] = None,
        fix_pretrained_bert_encoder_path: str = None,
        fix_pretrained_module_config: Dict[str, str] = None,
    ) -> "Trainer":
        """
        ...
        tensorboard_writer_._should_log_learning_rate = False
