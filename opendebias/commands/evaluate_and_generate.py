...
class DebiasedEvaluate(Subcommand):
    ...
    def add_subparser(self, parser: argparse._SubParsersAction) -> argparse.ArgumentParser:
        ...
        subparser.add_argument(
            "evaluate_file_config_files", type=json.loads, help="path to the config file describing the evaluation data"
        )
...
def evaluate_from_args(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    for eidx, eval_input_config_path in enumerate(args.evaluate_file_config_files):
...
def evaluate_on_dataset_config(model, eval_input_config, args, metrics, eidx):
    ...
    generator_tqdm = Tqdm.tqdm(iterator, desc=f"Evaluate on {dataset_reader.name}", total=len(data_loader))
    ...
    with torch.no_grad():
        if args.bias_only:
            ...
            metrics, ret = eval_on_dataset_bias_only(
                model, dataset_reader.name, dataset_reader, generator_tqdm, metrics, args, eidx)
        else:
    ...
    with open(pathlib.Path(predictions_output_folder) / f'{dataset_reader.name}.json', 'w') as f:
        json.dump(ret, f)
    return metrics
...
def eval_on_dataset_bias_only(model, dataset_name, dataset_reader, data_generator, metrics, args, eidx):
    ...
    for batch in data_generator:
        with amp.autocast(args.amp):
            ...
            all_ids.extend([b['id'] for b in batch['metadata']])
...
def eval_on_dataset(model, dataset_name, dataset_reader, data_generator, metrics, args, eidx):
    ...
    for batch in data_generator:
        with amp.autocast(args.amp):
            ...
            all_ids.extend([b['id']  for b in batch['metadata']])
            batch_outputs = model(**batch)
            if 'main_logits' in batch_outputs:
    ...
    ret['id'] = [str(id_) for id_ in all_ids]
